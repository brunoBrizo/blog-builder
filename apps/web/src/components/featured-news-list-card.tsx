...
import type { NewsListFeatured } from '../mocks/news';

type FeaturedNewsListCardProps = {
  featured: NewsListFeatured;
};

export function FeaturedNewsListCard({ featured }: FeaturedNewsListCardProps) {
  return (
    <article className="group relative flex flex-col lg:flex-row bg-white rounded-[2rem] border border-zinc-200/80 shadow-sm hover:shadow-md transition-all duration-500 mb-12 overflow-hidden">
      ...
      >
        <span className="sr-only">
          Read breaking news: {featured.listTitle}
        </span>
      </Link>

      <div className="lg:w-[45%] p-8 lg:p-12 flex flex-col justify-center order-2 lg:order-1 relative z-20 bg-white">
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-red-50 text-red-600 text-xs font-medium mb-6 w-fit border border-red-100/50">
          <Zap className="w-3.5 h-3.5" strokeWidth={1.5} aria-hidden />
          {featured.badgeLabel}
        </div>
        <h2 className="font-display text-3xl lg:text-5xl font-medium tracking-tight text-zinc-900 mb-4 group-hover:text-red-600 transition-colors leading-tight">
          {featured.listTitle}
        </h2>
        <p className="text-sm lg:text-lg font-light text-zinc-500 mb-8 leading-relaxed">
